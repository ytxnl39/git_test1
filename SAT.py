...
class Statistics:
    ...
    def __init__(self):
        ...
        # 存储输出已解决问题的统计信息的的文件路径
        self._output_statistics_file = ""
        
        # Path of the output assignment file which stores the satisfying assignment if the problem is satisfied, it is empty if the problem is UNSAT
        # 存储输出赋值结果的文件路径(unSAT=0)
        ...
        # Number of original clauses stored (The unary clauses are not stored and are directly used to get the assignment)
        # 除单子句以外存储的初始子句数量（单子句直接赋值）
        self._num_clauses = 0
    ...
    def print_stats(self):
        ...
        # Check if the result of the problem is SAT and if it is, then show the assignement file name
        if self._result == "SAT":
...
class AssignedNode:
    ...
    def __init__(self,var,value,level,clause):
        ...
        #推理出这个结果的子句，如果是被decide的，就设置为none
        self.clause = clause 
...
class SAT:
    ...
    def __init__(self,to_log,decider,restarter=None):
        ...
        Parameters:
            to_log: 标记solver是否要记录流程 a boolean (True/False) which indicates whether the solver should log the progress while solving the problem
            decider: 三种启发式决策选一 the decision heuristic used while deciding the next variable to be assigned and the value to be assigned to it in the _decide method
            restarter: 是否要重启 the restart strategy to be used by the SAT solver (None set by default, so if nothing is passed, restarting will not be used)
        ...
        # List of clauses
        # where each clause is stored as a list of literals as explained above.
        # 具体的每一句子句
        self._clauses = []

        # 2 - literal watching
        # Dictionary mapping [a literal] -> [the list of clauses the literal watches]
        # 文字->watch的子句列表
        self._clauses_watched_by_l = {}
        
        # Dictionary mapping [a clause] -> [the list of literals that watch the clause]
        # 子句->被watch的文字
        self._literals_watching_c = {}
        
        # Dictionary mapping the [variables] -> [their assignment nodes]
        ...
        # 变量->赋值结点（包含变量值，含有此变量的子句，设置变量的层级等信息）
        ...
        # 决策启发式 只能三选一 ORDERED, VSIDS or MINISAT 如果不是就报错
...
#需要判断输入的数值是否代表一个负文字.在输入文本中，我们用5来表示-2(共有3个元素的情况下).
...
#输入CNF文本的时候，吃一个子句处理并添加到clause database，比如如果是-2就要处理成5这样。
def add_clause(self,clause):
    #吃的输入子句是这样的列表 ['2', '3', '-1', '0']
    '''
    ...
    # OrderedDict's fromkeys method makes an dictionary from the elements of the list clause
    # and we again make a list from it to remove dupicates. (We could use set here but it does
    # not maintain the order and adds randomness)
    # 删去子句中的重复文字
    ...
    # case
    # 如果是单位子句，那么这个文字必为真，特殊处理
    if len(clause)==1:
        #文字lit 元素var 正负value_to_set
        # Get the literal
        lit = clause[0]
        ...
        if var not in self._variable_to_assignment_nodes:
            # 元素还没有被指派值  {元素->指派此元素值的结点}的字典里没有这个元素的key
            ...
            # 如果已经指派的值和现在想要的值冲突了，就是unSAT直接结束
    ...
    #处理每个文字，添加到clause_with_literals，维护VSIDS和MINISAT对应的列表
    for lit in clause:
        if lit[0]=='-':
...
def read_dimacs_cnf_file(self,cnf_filename):
    ...
    for line in cnf_file.readlines():
        ...
        # 以p开头句子指明变量个数和子句个数
            ...
            # 填入变量个数
            self._num_vars = int(line[2])  #填入变量个数
            ...
            # all literals by 0
            # 如果启发式决策用的是"VSIDS" 需要对每个文字(数量是变量的两倍)维护变量，这里做初始化
            # 列表生成式，+1是为了下标符合元素index 生成全0列表
            if self._decider == "VSIDS":
            ...
            #如果启发式决策用的是MINISAT，需要对每个变量维护scores和phase数组，这里做初始化
            if self._decider == "MINISAT":
